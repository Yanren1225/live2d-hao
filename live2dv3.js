...
class l2dViewer {
  constructor({ basePath, modelName, width, height, el, sizeLimit, mobileLimit }) {
    ...
    this.app = new PIXI.Application(width, height, { transparent: true,preserveDrawingBuffer:true })
    ...
    window.onresize = (event) => {
      ...
      this.startAnimation("breath","base")
    }
    this.isClick = false
    window.addEventListener('mousemove', (event) => {
      if (this.model) {
        this.model.inDrag = true
        const mouse_x = this.canvas.firstChild.offsetLeft - event.clientX + this.canvas.firstChild.clientWidth / 2
        const mouse_y = window.innerHeight - this.canvas.firstChild.offsetTop - event.clientY - this.canvas.firstChild.clientHeight * 2 / 3
        if (window.innerHeight - this.canvas.firstChild.offsetTop - this.canvas.firstChild.clientHeight * 2 / 3 < event.clientY){
          this.model.pointerY = - mouse_y / ( this.canvas.firstChild.clientHeight / 2 )
        }else{
          this.model.pointerY = - mouse_y / ( this.canvas.firstChild.clientHeight * 2 )
        }
        if (this.canvas.firstChild.offsetLeft + this.canvas.firstChild.clientWidth / 2 < event.clientX){
          this.model.pointerX = - mouse_x / ( this.canvas.firstChild.clientHeight * 2 )
        }else{
          this.model.pointerX = - mouse_x / ( this.canvas.firstChild.clientHeight / 2 )
        }
      }
  ...
  onUpdate (delta) {
    ...
    if (this.inDrag) {
      ...
      this.addParameterValueById('ParamEyeBallX', this.pointerX/10)
      this.addParameterValueById('ParamEyeBallY', -this.pointerY/5)
    }
